...
public class AuthController : ControllerBase
{
    ...
    public async Task<IActionResult> AdminSetup([FromForm] IFormCollection form)
    {
        try
        {
            // Obtener el UserId del token JWT
            ...
            if (logoFile != null && logoFile.Length > 0)
            {
                ...
                var logoPath = await _fileStorageService.SaveFileAsync(logoStream, logoFile.FileName, "branding");
                logoUrl = _fileStorageService.GetFileUrl(logoPath);
            }

            var faviconFile = form.Files.GetFile("faviconFile");
            if (faviconFile != null && faviconFile.Length > 0)
            {
                using var faviconStream = faviconFile.OpenReadStream();
                var faviconPath = await _fileStorageService.SaveFileAsync(faviconStream, faviconFile.FileName, "branding");
                faviconUrl = _fileStorageService.GetFileUrl(faviconPath);
            }

            var yapeQRFile = form.Files.GetFile("yapeQRFile");
            if (yapeQRFile != null && yapeQRFile.Length > 0)
            {
                using var yapeQRStream = yapeQRFile.OpenReadStream();
                var yapeQRPath = await _fileStorageService.SaveFileAsync(yapeQRStream, yapeQRFile.FileName, "payment-qr");
                yapeQRUrl = _fileStorageService.GetFileUrl(yapeQRPath);
            }

            var plinQRFile = form.Files.GetFile("plinQRFile");
            if (plinQRFile != null && plinQRFile.Length > 0)
            {
                using var plinQRStream = plinQRFile.OpenReadStream();
                var plinQRPath = await _fileStorageService.SaveFileAsync(plinQRStream, plinQRFile.FileName, "payment-qr");
                plinQRUrl = _fileStorageService.GetFileUrl(plinQRPath);
            }
            ...
            {
                var categoriesJson = form["categories"].ToString();
                if (!string.IsNullOrEmpty(categoriesJson))
                {
                    categories = JsonSerializer.Deserialize<List<string>>(categoriesJson) ?? new();
                }
            }

            // Procesar banner de home
            string? homeBannerImageUrl = null;
            var homeBannerFile = form.Files.GetFile("homeBannerImage");
            if (homeBannerFile != null && homeBannerFile.Length > 0)
            {
                using var homeBannerStream = homeBannerFile.OpenReadStream();
                var homeBannerPath = await _fileStorageService.SaveFileAsync(homeBannerStream, homeBannerFile.FileName, "home-banners");
            ...
            var command = new AdminSetupCommand
            {
                ...
                WhatSells = form["whatSells"].ToString(),
                IsVirtual = bool.TryParse(form["isVirtual"].ToString(), out var isVirtual) && isVirtual,
                SedeAddress = form["sedeAddress"].ToString(),
                SedeCity = form["sedeCity"].ToString(),
                SedeRegion = form["sedeRegion"].ToString(),
                SedePhone = form["sedePhone"].ToString(),
                LogoUrl = logoUrl,
                ...
                SystemUsers = form["systemUsers"].ToString() ?? "1-5",
                // Personalización de página
                HomeTitle = form["homeTitle"].ToString(),
                HomeSubtitle = form["homeSubtitle"].ToString(),
                HomeDescription = form["homeDescription"].ToString(),
                HomeBannerImageUrl = homeBannerImageUrl,
                // Información de Pago y Envío
                YapePhone = form["yapePhone"].ToString(),
                PlinPhone = form["plinPhone"].ToString(),
                YapeQRUrl = yapeQRUrl,
                PlinQRUrl = plinQRUrl,
                BankName = form["bankName"].ToString(),
                BankAccountType = form["bankAccountType"].ToString(),
                BankAccountNumber = form["bankAccountNumber"].ToString(),
                BankCCI = form["bankCCI"].ToString(),
                DeliveryType = form["deliveryType"].ToString() ?? "Ambos",
                DeliveryCost = decimal.TryParse(form["deliveryCost"].ToString(), out var deliveryCost) ? deliveryCost : null,
                DeliveryZones = form["deliveryZones"].ToString(),
                CreateCashier = bool.TryParse(form["createCashier"].ToString(), out var createCashier) && createCashier,
                CashierEmail = form["cashierEmail"].ToString(),
                CashierPassword = form["cashierPassword"].ToString(),
                CashierFirstName = form["cashierFirstName"].ToString(),
                CashierLastName = form["cashierLastName"].ToString(),
                CashierDni = form["cashierDni"].ToString()
            };
        ...
        catch (Exception ex)
        {
            ...
            return StatusCode(500, new { message = "Error al procesar la configuración", error = ex.Message });
        }
    }
