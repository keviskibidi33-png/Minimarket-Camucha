...
public class SedesController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAll([FromQuery] bool? soloActivas)
    {
        try
        {
            ...
            if (!result.Succeeded)
            {
                // En lugar de 500, devolver lista vacía para evitar errores en frontend
                _logger.LogWarning("Error al obtener sedes: {Error}", result.Error);
                return Ok(Array.Empty<SedeDto>());
            }
        ...
        {
            _logger.LogError(ex, "Excepción al obtener sedes");
